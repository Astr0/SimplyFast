...
namespace SimplyFast.Reflection.Tests
{
    public class TypeExTests
    {
        ...
        {
            Assert.Equal(typeof(IEnumerable<string>),
                TypeEx.FindGenericType(typeof(IEnumerable<>), typeof(ITestEnumerable)));
            Assert.Equal(typeof(ICollection<string>),
                TypeEx.FindGenericType(typeof(ICollection<>), typeof(List<string>)));
        ...
        [Fact]
        public void HierarchyWorks()
        {
            Assert.Equal(Type.EmptyTypes, ((Type) null).Hierarchy());
            Assert.Equal(new[] {typeof(object)}, typeof(object).Hierarchy());
            Assert.Equal(new[] {typeof(void), typeof(ValueType), typeof(object)}, typeof(void).Hierarchy());
            Assert.Equal(new[] {typeof(SomeClass3), typeof(SomeClass2), typeof(SomeClass1), typeof(object)},
                typeof(SomeClass3).Hierarchy());
        }
